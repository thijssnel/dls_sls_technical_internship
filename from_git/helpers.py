...
def get_contributios_prior(g1_autocorrelation, time, s_space, betaPrior, alpha, weights=None):
    """
    Input:
        g1_autocorrelation : array (n_points x n_datasets)
        time               : array (n_points,)
        s_space            : array, decay rates for kernel
        betaPrior          : array (n_datasets,)
        alpha              : float or list of floats
        weights            : optional array (n_points x n_datasets) or (n_points,)
    Returns:
        contributions, residuals, penaltyNorms
    """

    # Zorg dat g1 altijd 2D is
    # g1_autocorrelation = np.atleast_2d(g1_autocorrelation)
    # nPoints, nDatasets = g1_autocorrelation.shape


    # Maak alpha een lijst als nodig
    # if not isinstance(alpha, (list, np.ndarray)):
    #     alphaList = [alpha] * nDatasets
    # else:
    alphaList = alpha
    ...
    # # Iterate over alle datasets
    # for i in range(nDatasets):
    g1temp = g1_autocorrelation[:]

        # Filter NaNs
    valid_mask = np.isfinite(g1temp)
    if np.sum(valid_mask) < 5:
        contributions.append(np.zeros_like(s_space))
        residuals.append(np.nan)
        penaltyNorms.append(np.nan)
        

    g1Filtered = g1temp[valid_mask]
    Afiltered = A[valid_mask]

    # Weights
    if weights is None:
        weightsIdx = np.ones_like(g1Filtered)
    elif weights.ndim == 1:
        weightsIdx = weights[valid_mask]
    else:
        weightsIdx = weights[valid_mask]
    
    try:
        # Hier moet jouw eigen Tikhonov-reg functie komen:
        cont, residual, penaltyNorm = tikhonov_Phillips_reg(
            Afiltered, alphaList, g1Filtered, weightsIdx
        )
    except Exception as e:
        print(f"⚠️ Dataset {i}: fit mislukt ({e})")
        cont = np.zeros_like(s_space)
        residual = np.nan
        penaltyNorm = np.nan
    ...
    return cont, np.array(residual), np.array(penaltyNorm)
